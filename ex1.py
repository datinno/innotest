...
target_df.set_index("t", inplace=True)
...
def Check3(df: pd.DataFrame, pre_price):
    ...
    filter_df = df[(df[cols]>max) | (df[cols]<min)].dropna(how="all").dropna(axis=1, how="all")

    return filter_df

def Check4(source: pd.DataFrame, target: pd.DataFrame):

    result = source.reset_index().merge(target.reset_index(), indicator = True, how='outer').set_index("t").loc[lambda x : x['_merge']!='both']

    return result


result = Check4(df, target_df)
